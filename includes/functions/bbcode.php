...
function load_bbcode_template()
{
    ...
    // 使用 preg_replace_callback 处理模板
    $tpl = preg_replace_callback(
        '#<!-- BEGIN (.*?) -->(.*?)<!-- END (.*?) -->#',
        function ($r) use (&$bbcode_tpls) { // 使用 & 确保引用
            return $bbcode_tpls[$r[1]] = $r[2];
        },
        $tpl
    );

    // 如果需要执行的代码在模板中，可以考虑其他安全的方法来解析
    // 例如：将模板字符串解析成 PHP 数组或对象，而不是 eval
	//eval($tpl); //这里有错误
    return $bbcode_tpls;
}
...
function bbencode_first_pass($text, $uid)
{
    ...
    // 修改这一行，使用 preg_replace_callback
